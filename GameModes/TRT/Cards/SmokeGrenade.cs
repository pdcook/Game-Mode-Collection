...
namespace GameModeCollection.GameModes.TRT.Cards
{
    ...
    {    
        ...
        {
            return "A standard non-lethal smoke grenade. Press [item 3] to throw it.";
        }
    ...
    public class A_SmokeGrenade : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                used = true;
                GameModeCollection.instance.StartCoroutine(SmokeGrenadeHandler.MakeSmokeGrenadeHandler(this.Player.playerID, ThrowSpeed * this.Player.data.weaponHandler.gun.shootPosition.forward, this.Player.data.weaponHandler.transform.position + this.Player.data.weaponHandler.gun.shootPosition.forward, Quaternion.identity));
